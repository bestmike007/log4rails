...
RSpec.describe "Log4r" do
  ...
  it "tests formatter with ndc" do
    ...
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {} % no NDC\s{34}$/) {
      l.info "no NDC"
    }
    Log4r::NDC.push("start")
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {start} % start NDC\s{31}$/) {
      l.info "start NDC"
    }
    Log4r::NDC.push("finish")
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {start finish} % start finish NDC\s{24}$/) {
  ...
  it "tests formatter with GDC" do
    ...
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {.+rspec} % GDC default\s{29}$/) {
      l.info "GDC default"
    }
    Log4r::GDC.set("non-default")
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {non-default} % GDC non-default\s{25}$/) {
  ...
  it "tests formatter with MDC" do
    ...
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {} % no user\s{33}$/) {
      l.info "no user"
    }
    Log4r::MDC.put("user","bestmike007")
    o.expect_log(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}   INFO \[test::this::that::other\]other {bestmike007} % user bestmike007\s{24}$/) {
